...
const VideoStyleGenerator: React.FC = () => {
    ...
    const handleGenerate = async () => {
        if (!prompt || !imageFile) {
            setError("Please provide a prompt and a style image.");
            return;
        }
        setLoading(true);
        setError(null);
        ...
        try {
            setLoadingMessage('Preparing style image...');
            ...
            let operation = await ai.models.generateVideos({
                ...
                prompt: `${prompt}, in the artistic style of the reference image.`,
                image: {
    ...
    const content = (
         <div className="flex flex-col space-y-4 pt-4">
            <p className="text-gray-400 -mt-2 mb-2">Upload a style image and describe a scene. The AI will generate a video influenced by your image's artistic style.</p>
            <div className="aspect-video bg-gray-900/50 rounded-lg flex items-center justify-center border-2 border-dashed border-gray-700 p-2">
             ...
             <label className="text-sm font-medium text-gray-300">Style Image</label>
             <input
                type="file"
                accept="image/*"
            ...
            >
                {loading ? <><Spinner size="h-5 w-5 -ml-1 mr-3" /> Generating...</> : 'Generate Styled Video'}
            </button>
...
export default VideoStyleGenerator;